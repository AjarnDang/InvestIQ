...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    const tt = setTimeout(updateSummaryArrows, 120);
  ...
  const onSummaryMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        >
          ...
          <div className="shrink-0 flex flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm min-w-[120px] cursor-pointer hover:border-indigo-300 hover:shadow-md transition-all group">
            <StatCard
              ...
              value={`$${cashUsd.toLocaleString("en-US", {
            ...
            <p className="mt-1 text-[10px] text-slate-400 tabular-nums">
              1 USD ≈{" "}
              {fxUsdThb.toLocaleString("en-US", { maximumFractionDigits: 4 })}{" "}
              THB
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          <Card className="xl:col-span-2">
            <CardHeader>
              <CardTitle>
                {t("dashboard.performance")} (30 {locale === "th" ? "วัน" : "Days"})
              </CardTitle>
            </CardHeader>
