  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-1">
                <span className="text-2xl font-black text-slate-900">{sym}</span>
                {detail.exchange && (
              ...
              <p className="text-sm text-slate-500 truncate max-w-xs">{detail.name}</p>
            ...
            <div className="flex flex-row sm:flex-col items-end gap-3 sm:gap-1 shrink-0">
              <div className="text-right">
                <p className="text-2xl font-black tabular-nums text-slate-900">
                  {detail.currency === "THB" ? "฿" : "$"}{fmt(detail.price)}
                </p>
                <div className={cn("flex items-center justify-end gap-1 text-sm font-bold tabular-nums", positive ? "text-emerald-600" : "text-red-500")}>
                  {positive ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                  {positive ? "+" : ""}{fmt(detail.change)} ({positive ? "+" : ""}{fmt(detail.changePercent)}%)
                </div>
              </div>
        ...
        {/* Day range bar */}
        {detail.dayLow != null && detail.dayHigh != null && detail.dayLow !== detail.dayHigh && (
          <div className="px-5 md:px-6 pb-4">
            <div className="flex items-center gap-3 text-xs text-slate-500">
              ...
              <div className="relative flex-1 h-1.5 rounded-full bg-slate-100">
                <div
                  className={cn("h-full rounded-full", positive ? "bg-emerald-400" : "bg-red-400")}
                  style={{
